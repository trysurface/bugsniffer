...
- **Roadmap DB (feature requests → `Type: Idea`):** `38444c625b9f8045bc63cee7262dfff7` / data source `6eb44c62-5b9f-833a-b790-87bc24a20eeb`; view "Ideas from #product-feedback" filters on `Type = Idea`. Page URL in `config.notion.roadmap.pageUrl`.
...
| `ROADMAP_DATABASE_ID` | ❌ | Override Roadmap database for feature-request Ideas (default: 38444c625b9f8045bc63cee7262dfff7) |
...
- Larger feature requests are filed on the Roadmap as `Type: Idea` (`draftIdea()` write-up + `createRoadmapIdea()`: Slack link callout, reporter, verbatim message, uploaded screenshots) with a one-off reply linking the page — no dedup, no thread tracking
...
- The Notion integration must be shared with the Bug Tracker, Eng Task Tracker **and Roadmap** databases (database ⋯ → Connections → Add) — unshared → `object_not_found` and feature requests silently fail to file
- Always use `client.chat.getPermalink()` for Slack URLs — never construct them manually (workspace subdomain varies)

## Changelog

- **2026-08-25** — Feature requests now get filed on the product Roadmap as `Type: Idea` (shows in the "Ideas from #product-feedback" view) instead of a "add it yourself" reply. `draftIdea()` (classifier) produces Summary/Problem/Proposal/Open-questions; `createRoadmapIdea()` (notion) builds the page with Slack permalink, reporter, verbatim message + uploaded images. New `ROADMAP_DATABASE_ID`; integration must be shared with the Roadmap DB.
